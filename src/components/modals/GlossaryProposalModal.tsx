  ...
  useEffect(() => {
    if (isOpen) {
      if (proposal) {
        ...
        reset({ term: '', definition: '', context: '', scripturalWitness: '', restorationNote: '', tags: '' });
      }
    }
      ...
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg relative border modal-bg-pattern flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={handleSubmit(handleSave)} className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-start gap-4 pr-8">
                ...
                <div>
                    <h2 className="text-xl font-bold text-foreground">{proposal ? 'Edit' : 'Propose a'} Glossary Change</h2>
            ...
            <div className="flex-grow overflow-y-auto p-6 space-y-4">
                ...
                <div className="space-y-2">
                 ...
                 <div className="space-y-2">
                    ...
                    <Controller name="context" control={control} render={({ field }) => <Textarea id="context" {...field} rows={2} className="bg-background/50" placeholder="Why is this term important?" />} />
                </div>
                 <div className="space-y-2">
                    <Label htmlFor="scripturalWitness">Scriptural Witness (Optional)</Label>
                    <Controller name="scripturalWitness" control={control} render={({ field }) => <Input id="scripturalWitness" {...field} className="bg-background/50" placeholder="e.g., Genesis 1:14; Jubilees 6:32"/>} />
                </div>
                 <div className="space-y-2">
                    <Label htmlFor="restorationNote">Restoration Note (Optional)</Label>
                    <Controller name="restorationNote" control={control} render={({ field }) => <Textarea id="restorationNote" {...field} rows={2} className="bg-background/50" placeholder="Why is restoring this term important?"/>} />
                </div>
                 <div className="space-y-2">
